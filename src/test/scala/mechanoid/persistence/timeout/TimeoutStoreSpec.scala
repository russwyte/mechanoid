  ...
  def spec = suite("TimeoutStore")(
    suite("schedule")(
      test("creates a new timeout") {
        ...
        for
          timeout <- store.schedule("fsm-1", "WaitingForPayment", deadline)
        yield assertTrue(
    ...
    suite("cancel")(
      ...
      test("returns false for non-existent timeout") {
        val store = new InMemoryTimeoutStore[String]()
        for
          cancelled <- store.cancel("non-existent")
      ...
      test("respects limit parameter") {
        ...
        for
          _ <- ZIO.foreach(1 to 10)(i =>
            store.schedule(s"fsm-$i", "S", now.minusSeconds(i.toLong))
          )
    ...
    suite("complete")(
      ...
      test("returns false for non-existent timeout") {
        val store = new InMemoryTimeoutStore[String]()
        for
          completed <- store.complete("non-existent")
