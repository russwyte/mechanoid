...
object DocumentWorkflowApp extends ZIOAppDefault:
  ...
  override def run: ZIO[Any, Any, Unit] =
    for
      ...
      // Show the DSL syntax
      _ <- Console.printLine("DSL Definition (suite-style):")
      _ <- Console.printLine("  build[DocumentState, DocumentEvent](")
      _ <- Console.printLine("    Draft via SubmitForReview to PendingReview,")
      _ <- Console.printLine("")
      _ <- Console.printLine("    // Group transition - one line for ALL InReview states!")
      ...
      _ <- Console.printLine("    UnderReview via RequestChanges to ChangesRequested,")
